...
export const RecordsTable: React.FC<RecordsTableProps> = ({ records, onEdit, onDelete, showExtended = false }) => {
  ...
  return (
    <div className="table-container mt-6 sm:mt-8 shadow-sm">
        ...
        <table className="data-table">
          <thead>
            <tr className="whitespace-nowrap">
              ...
              {showActions && <th className="text-right px-4">Actions</th>}
            </tr>
          </thead>
          <tbody>
            {records.length === 0 ? (
              ...
              records.map((record) => {
                ...
                return (
                  <tr key={record.id} className="whitespace-nowrap">
                    ...
                    <td>
                      <span className="px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200">
                        {record.plant}
                      </span>
                    </td>
                    <td className="tabular-nums text-right text-xs sm:text-sm">
                    ...
                    <td className={`tabular-nums font-bold text-right text-xs sm:text-sm ${gapStock < 0 ? 'text-rose-600' : 'text-slate-700'}`}>
                      {gapStock.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td className="text-right">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-bold ${getUtilColor(percentage)}`}>
                        {percentage.toFixed(2)}%
                      </span>
                    </td>
                    {showExtended && (
                      <>
                        <td className="text-[10px] sm:text-xs text-slate-500 capitalize">{record.created_by}</td>
                        <td className="text-[9px] sm:text-[10px] text-slate-400 tabular-nums">
                    ...
                    {showActions && (
                      <td className="text-right px-4">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2 text-slate-400">
                          {canEdit ? (
                            <>
                              {onEdit && (
                                <button 
                                  onClick={() => onEdit(record)}
                                  className="p-1 sm:p-1.5 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"
                                  title="Edit Record"
                                >
                              ...
                              {onDelete && (
                                <button 
                                  ...
                                  className="p-1 sm:p-1.5 hover:text-rose-600 hover:bg-red-50 rounded-lg transition-all"
                                  title="Delete Record"
                                >
                            ...
                            <span className="text-[9px] uppercase font-bold tracking-wider text-slate-300 px-1.5 py-0.5 bg-slate-50 rounded border border-slate-100">
                              Locked
                            </span>
                          )}
                        </div>
